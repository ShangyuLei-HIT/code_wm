...
def compute_lewm_losses(
    ...
    """Compute LeWM losses, skipping SIGReg entirely when disabled."""
    pred_loss = (pred_emb - tgt_emb).pow(2).mean()
    output = {'pred_loss': pred_loss}
    if float(sigreg_weight) == 0.0:
        output['loss'] = pred_loss
        return output

    sigreg_loss = sigreg(emb.transpose(0, 1))
    output['sigreg_loss'] = sigreg_loss
    output['loss'] = pred_loss + float(sigreg_weight) * sigreg_loss
    return output
...
def run(cfg):
    ...
    if checkpoint_path is not None:
        ...
        checkpoint_path = (
            legacy_checkpoint if legacy_checkpoint.exists() else None
        )
    manager = spt.Manager(
        trainer=trainer,
        module=world_model,
        ...
        weights_only=False if checkpoint_path is not None else True,
    )
