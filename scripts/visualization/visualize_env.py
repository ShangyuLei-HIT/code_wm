...
def img_transform():
    transform = transforms.Compose(
        [
            ...
            # Match scripts/train/lewm.py (stable-pretraining ImageNet stats).
...
def prepare_info(info_dict, process, transform):
    ...
    for k, v in info_dict.items():
        ...
        # Collapse every leading dimension before the image dimensions.  Vector
        # env resets return (E, H, W, C), while trajectory-shaped inputs may be
        # (E, T, H, W, C); preserve both layouts when restoring the result.
        if k in transform:
            leading_shape = None
            if is_numpy or torch.is_tensor(v):
                if v.ndim >= 3:
                    leading_shape = v.shape[:-3]
            ...
            if leading_shape:
                v = v.reshape(*leading_shape, *v.shape[1:])
