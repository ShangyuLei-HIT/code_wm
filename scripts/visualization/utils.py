...
class LeWMAdapter(nn.Module):
    ...
    def encode(self, info, pixels_key='pixels', target='emb'):
        ...
        if encode_info['pixels'].dim() == 4:
            encode_info['pixels'] = encode_info['pixels'].unsqueeze(1)
        if (
        ...
        info.update(result)
        info[target] = result['emb']
        info['embed'] = result['emb']
        return info
...
class PreJEPAAdapter(nn.Module):
    ...
    def encode(self, info, pixels_key='pixels', target='emb'):
        if info[pixels_key].dim() == 4:
            info[pixels_key] = info[pixels_key].unsqueeze(1)
        emb_keys = [
            k
