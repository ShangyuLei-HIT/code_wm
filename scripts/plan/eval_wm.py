...
def seed_evaluation(seed: int) -> None:
    """Make episode selection and stochastic CEM sampling reproducible."""
    random.seed(seed)
    np.random.seed(seed)
...
def run(cfg: DictConfig):
    ...
    if cfg.output.get('directory'):
        results_path = Path(cfg.output.directory).expanduser().resolve()
    ...
    manifest_path = cfg.eval.get('manifest_path')
    if manifest_path:
        manifest = json.loads(
            Path(manifest_path).expanduser().resolve().read_text()
        )
        entries = manifest['entries']
        if len(entries) != int(cfg.eval.num_eval):
            raise ValueError(
                'manifest length differs from eval.num_eval: '
                f'{len(entries)} != {cfg.eval.num_eval}'
            )
        random_episode_indices = np.asarray(
            [int(item['row_index']) for item in entries], dtype=np.int64
        )
        eval_episodes = np.asarray(
            [int(item['episode_idx']) for item in entries], dtype=np.int64
        )
        eval_start_idx = np.asarray(
            [int(item['start_step']) for item in entries], dtype=np.int64
        )
        actual_episodes = dataset.get_col_data(col_name)[
            random_episode_indices
        ]
        actual_steps = dataset.get_col_data('step_idx')[
            random_episode_indices
        ]
        if not np.array_equal(actual_episodes, eval_episodes):
            raise RuntimeError('manifest episode ids do not match dataset')
        if not np.array_equal(actual_steps, eval_start_idx):
            raise RuntimeError('manifest start steps do not match dataset')
    else:
        ...
        random_episode_indices = np.sort(
            valid_indices[random_episode_indices]
        )
    ...
    with autocast_ctx:
        metrics = world.evaluate(
            ...
            video=results_path if bool(cfg.eval.get('video', True)) else None,
        )
    end_time = time.time()
    ...
    json_payload = {
        ...
        'row_indices': random_episode_indices.tolist(),
    ...
    mode = 'a' if bool(cfg.output.get('append', True)) else 'w'
